...
class KISService:
    def __init__(self, mode: str, app_key: str, app_secret: str, account_no: str) -> None:
        self.mode = mode
    ...
    def connect(self) -> None:
        self._auth = KISAuth(mode=self.mode, app_key=self.app_key, app_secret=self.app_secret)
    ...
    async def buy(self, ticker: str, qty: int | float, exchange: str = "NASD", price: float = 0.0) -> dict:
        if _is_domestic(ticker):
            return await place_domestic_order(self.auth, self.account_no, ticker, "buy", int(qty))
        return await place_overseas_order(self.auth, self.account_no, ticker, "buy", qty, exchange, price=price)

    async def sell(self, ticker: str, qty: int | float, exchange: str = "NASD", price: float = 0.0) -> dict:
        if _is_domestic(ticker):
            return await place_domestic_order(self.auth, self.account_no, ticker, "sell", int(qty))
        return await place_overseas_order(self.auth, self.account_no, ticker, "sell", qty, exchange, price=price)

    async def get_balance(self) -> dict:
        return await get_domestic_balance(self.auth, self.account_no)

    async def calc_buy_qty(self, ticker: str, price: float, fraction: float = 1.0) -> int | float:
        if price <= 0:
            return 0
        if _is_domestic(ticker):
            total_qty = await get_domestic_orderable_qty(self.auth, self.account_no, ticker)
            return math.floor(total_qty * fraction)
        cash = await get_overseas_available_cash(self.auth, self.account_no, ticker, price)
        return math.floor(cash * fraction * 0.98 / price)

    async def get_holding_qty(self, ticker: str, exchange: str = "NASD") -> int:
        try:
            if _is_domestic(ticker):
                data = await get_domestic_balance(self.auth, self.account_no)
                ...
                for exch in [exchange, "NYSE", "AMEX", "NASD"]:
                    try:
                        data = await get_overseas_balance(self.auth, self.account_no, exch)
                        for row in data.get("output1") or []:
